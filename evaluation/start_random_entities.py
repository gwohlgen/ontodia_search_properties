...
def do_evaluation(random_entities, skip_ids=False, skip_where_alias_same_as_label=False):
    ...
    for entity in random_entities:
        ...
        for alias, label in alias_labels_map.iteritems():
            ...
            if not property_found_in_entity(entity, label):
                continue

            # format: alias is a string, label is a list of strings
            close_props = helpers.get_closest_properties(alias, model, entity['properties'], w2v_plist, properties, debug_lookup=False, num_sugg = 100)

            if close_props:
                num_found += 1
                # helpers.pprint_closest_properties(alias + " -- label: " + str(label), close_props, properties)
                eval_data[entity['label'] + '___' + alias] = (label, close_props) 
            else:
                num_not_found += 1
...
if __name__=="__main__":
    ...
    random_entities = get_random_entities(num=1000)
    print stats_avg_num_properties(random_entities)

    do_evaluation(random_entities)
